...
class EmptySection:
    ...
    def blockstates(self, palette: Tuple[Block] = None) -> array.array:
        """
        Build the packed blockstates array for the section.
        Optimized for small palettes:
          - Use palette.index (fast for small lists)
          - Avoid intermediate Python list (append directly to array)
          - Cache frequently used functions/variables locally
        """
        palette = palette or self.palette()
        bits = max((len(palette) - 1).bit_length(), 4)

        states = array.array('Q')
        append_states = states.append
        bin_append_local = bin_append

        # precompute air index once
        try:
            air_index = palette.index(self.air)
        except ValueError:
            air_index = 0

        pal = palette  # local reference
        idx = pal.index
        ...
        for block in self.blocks:
            ...
            if current_len + bits > 64:
                append_states(current)
                current = index
                current_len = bits
            else:
                current = bin_append_local(index, current, length=current_len)
                current_len += bits
