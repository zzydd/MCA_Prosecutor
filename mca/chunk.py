...
class Chunk:
    ...
    __slots__ = ('x', 'z', 'version', 'status', 'data', 'tile_entities', 'entities')
    ...
    def __init__(self, nbt_data: nbt.NBTFile):
        ...
        if self.version < 1451:
            ...
            self.entities = self.data.get('Entities', None)
            self.tile_entities = self.data.get('TileEntities', None)
            self.status = "full"

        elif 1451 <= self.version < 2844:
            # 1.13 ~ 1.18 的版本
            self.data = nbt_data['Level']
            self.x = self.data['xPos'].value
            self.z = self.data['zPos'].value
            self.status = self.data['Status'].value
            self.entities = self.data.get('Entities', None)
            self.tile_entities = self.data.get('TileEntities', None)
        else:
            # 高于 1.18 的版本
