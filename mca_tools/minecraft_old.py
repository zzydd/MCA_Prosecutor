...
"""
本工具是对于 Minecraft 1.13 扁平化前的兼容层
包含方块ID和注册名的转化、适用于老版本的方块替换等工具
"""

def convert_target_dict(target_dict: dict) -> dict:
    """
    将 target_dict 转化成 target_id_dict
    {"minecraft:stone": "minecraft:air"} > {1: 0}
    """
    out = {}
    for src, dst in target_dict.items():
        sid = Old_Minecraft_Name_Map.get(src)
        did = Old_Minecraft_Name_Map.get(dst)
...
def replace_block(section, target_dict):
    """适用于 1.13- 版本的方块替换工具"""
    # 转化目标列表
    target_id_dict = convert_target_dict(target_dict)
    # 获取方块数据
    blocks = section["Blocks"]
    add = section.get("Add")  # 可能不存在
    ...
    # 有Add数据: 有ID>255的方块
        ...
        for i in range(4096):
            ...
            if nid is not None:
                ...
                new_high = nid >> 8
                if i & 1:
                    add[add_index] = (add_byte & 0x0F) | (new_high << 4)
                else:
                    add[add_index] = (add_byte & 0xF0) | new_high

    return section


# 方块ID 和 注册名 映射表
Old_Minecraft_ID_Map = {
    # 方块ID
    0: 'minecraft:air',
    1: 'minecraft:stone',
    2: 'minecraft:grass',
...
Old_Minecraft_Name_Map = {v: k for k, v in Old_Minecraft_ID_Map.items()}

# AI真是太好用了你知道吗(～￣▽￣)～
# 如果用能工智人来写这个表格不知道要多久哈哈